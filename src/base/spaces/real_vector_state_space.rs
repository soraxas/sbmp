...
//             dimension: 0,
//             state_bytes: 0,
//             bounds: RealVectorBounds { low: vec![], high: vec![] },
//             dimension_names: vec![],
...
//         bounds.check();
//         assert_eq!(bounds.low.len(), self.dimension, "Bounds do not match dimension of state space");
//         self.bounds = bounds;
//     }
...
//         self.bounds.low.iter().zip(&self.bounds.high)
...
//             if state[i] > self.bounds.high[i] {
//                 state[i] = self.bounds.high[i];
//             } else if state[i] < self.bounds.low[i] {
//                 state[i] = self.bounds.low[i];
//             }
//         }
//     }
...
//         for i in 0..self.dimension {
//             if state[i] - EPSILON > self.bounds.high[i] || state[i] + EPSILON < self.bounds.low[i] {
//                 return false;
//             }
//         }
//         true
...
//         state1.iter().zip(state2)
//             .all(|(s1, s2)| (s1 - s2).abs() <= EPSILON * 2.0)
//     }
...
//             state[i] = self.rng.gen_range(self.space.bounds.low[i]..=self.space.bounds.high[i]);
//         }
//     }

//     pub fn sample_uniform_near(&mut self, state: &mut [f64], near: &[f64], distance: f64) {
//         for i in 0..self.space.dimension {
//             state[i] = self.rng.gen_range(
//                 (near[i] - distance).max(self.space.bounds.low[i])..=
//                 (near[i] + distance).min(self.space.bounds.high[i])
//             );
//         }
//     }
...
//         for i in 0..self.space.dimension {
//             let mut v = self.rng.sample(rand_distr::Normal::new(mean[i], std_dev).unwrap());
//             if v < self.space.bounds.low[i] {
//                 v = self.space.bounds.low[i];
//             } else if v > self.space.bounds.high[i] {
//                 v = self.space.bounds.high[i];
//             }
//             state[i] = v;
//         }
//     }
